...
if ($_POST["kind"] == "jumkat") {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $kat = ucwords($row["kat"]);
            $qty = $row["jum"];
            $kal .= "<a href='kategori.php?idk=$idkat' class='border pl-1 pr-1 mr-1 ml-1 mb-1 mt-1 rounded text-dark'>$kat <span id='k_$idkat' class='badge badge-secondary'>$qty</span></a>";
        }
    } else {
